 ...
 */
 ...
 (function(gap) {	
	...
	ScanditSDK.prototype.scan = function(success, fail, appKey, options) {
		//for Community and trial version
		if (isDeviceOffline){
			if (typeof fail == "function"){
				fail("Need internet connection");
			}
			return;
		}
		if (!options){
	    ...
	    return gap.exec(function(result){
	    	if (typeof success == "function"){
				 ...
				 //iOS version
					 ...
					 resArr = result.split("|");
					 success(resArr[1], resArr[0]);
				 }
			}
	    }, fail, 'ScanditSDK', 'scan', [appKey, options]);
	};	
	
	gap.addConstructor(function () {
        if (gap.addPlugin) {
            gap.addPlugin("ScanditSDK", new ScanditSDK());
        } else {
            if (!window.plugins) {
                window.plugins = {};
            }

            window.plugins.ScanditSDK = new ScanditSDK();
        }
